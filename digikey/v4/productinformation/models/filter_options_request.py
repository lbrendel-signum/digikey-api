...
class FilterOptionsRequest(object):
    ...
    def market_place_filter(self, market_place_filter):
        ...
        if market_place_filter not in allowed_values:
            raise ValueError(
                "Invalid value for `market_place_filter` ({0}), must be one of {1}".format(  # noqa: E501
                    market_place_filter, allowed_values
                )
            )
    ...
    def search_options(self, search_options):
        ...
        if not set(search_options).issubset(set(allowed_values)):
            raise ValueError(
                "Invalid values for `search_options` [{0}], must be a subset of [{1}]".format(  # noqa: E501
                    ", ".join(map(str, set(search_options) - set(allowed_values))),  # noqa: E501
