...
from digikey.v3.batchproductdetails import (
...
from digikey.v3.productinformation import (
    DigiReelPricing,
...
class DigikeyApiWrapper:
    def __init__(self, wrapped_function, module):
        ...
        self._digikeyApiToken = digikey.oauth.oauth2.TokenHandler(
            version=3, sandbox=self.sandbox
        ).get_access_token()
...
def manufacturer_product_details(*args, **kwargs) -> KeywordSearchResponse:
    client = DigikeyApiWrapper(
        "manufacturer_product_details_with_http_info", digikey.v3.productinformation
    )
...
def salesorder_history(*args, **kwargs) -> [SalesOrderHistoryItem]:
    ...
    if (
        "start_date" in kwargs
        and type(kwargs["start_date"]) == str
        and "end_date" in kwargs
...
def batch_product_details(*args, **kwargs) -> BatchProductDetailsResponse:
    client = DigikeyApiWrapper(
        "batch_product_details_with_http_info", digikey.v3.batchproductdetails
    )
